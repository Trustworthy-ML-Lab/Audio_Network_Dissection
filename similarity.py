...
def soft_wpmi(clip_feats, target_feats, top_k=100, a=10, lam=1, device='cuda',
    ...
    top_k = min(top_k, target_feats.size(0))
    with torch.no_grad():
        torch.cuda.empty_cache()
    ...
    with torch.no_grad():
        ...
        for orig_id in range(target_feats.shape[1]):
            ...
            for i in range(math.ceil(clip_indices.shape[1]/batch)):
                ...
                diff = (target-reorg)
                curr_errors.append(torch.nanmean(torch.abs(diff)**p, dim=0, keepdim=True)/baseline_diff)
...
def max_sim(info_score, concepts, sim):
    return torch.amax(info_score)

def max_minus_sec(info_score, concepts, sim):
    top = torch.topk(info_score, k=2)
    return top.values[0] - top.values[1]

def max_minus_other(info_score, concepts, sim):
    return 2 * torch.amax(info_score) - torch.sum(info_score)

def weighted_diff(info_score, concepts, sim):
    max_val, max_id = torch.topk(info_score, k=1)
    penalty = 0
    for i in range(len(info_score)):
        if i != max_id:
            penalty += info_score[i] * sim[concepts[max_id]][concepts[i]]
